...
export const show = async (req, res) => {
  try {
    ...
    if (!kb) return res.status(404).json({ status: false, message: 'Not found' });

    res.json({ status: true, data: kb });
  } catch (err) {
    res.status(500).json({ status: false, error: err.message });
  }
};
...
export const store = async (req, res) => {
  try {
    ...
    const newKb = await knowledgeBaseModel.addNewKb({ judul_kb});
...
export const update = async (req, res) => {
 ...
 if (!updatedKb) return res.status(404).json({ status: false, message: 'Not found' });
